...
class textbox(object):
    ...
    def create_textbox(self):
        pygame.draw.rect(self.screen, RED, [self.x_pos*self.x_offset, self.y_pos*self.y_offset, 24*self.x_offset, 5*self.y_offset], 10) 
    ...
    def blitz(self):
        for i in range (5):
            self.screen.blit(self.rendered.text[i], [self.x_pos*self.x_offset, self.y_pos*self.y_offset])
            self.y_pos += 1
